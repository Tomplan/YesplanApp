...
struct Event_Id: Codable {
    ...
    func printEvent_Id() {
        ...
        if let defaultscheduledescription = self.defaultscheduledescription {
            print("\t defaultscheduledescription: \(defaultscheduledescription)") }
        if let defaultschedulestarttime = self.defaultschedulestarttime {
            print("\t defaultschedulestarttime: \(defaultschedulestarttime)") }
        if let defaultscheduleendtime = self.defaultscheduleendtime {
            print("\t defaultscheduleendtime: \(defaultscheduleendtime)") }
        if let defaultschedulestart = self.defaultschedulestart {
            print("\t defaultschedulestart: \(defaultschedulestart)") }
        if let defaultscheduleend = self.defaultscheduleend {
            print("\t defaultscheduleend: \(defaultscheduleend)") }
    }
}

extension Event_Id {
    init?(data: Data) {
        guard let me = try? JSONDecoder().decode(Event_Id.self, from: data) else { return nil }
        self = me
    }
